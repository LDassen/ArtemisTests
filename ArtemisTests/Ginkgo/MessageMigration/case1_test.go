...
var _ = ginkgo.Describe("MessageMigration Test", func() {
	...
	// Map to store the association between broker names and pod names
	var brokerPodMap map[string]string
	...
	ginkgo.BeforeEach(func() {
		...
		// Initialize brokerPodMap
		brokerPodMap = make(map[string]string)
	})
	...
	ginkgo.It("should send, delete, and check messages", func() {
		...
		for {
			for broker, receiver := range receivers {
				...
				if string(msg.GetData()) == messageText {
					...
					// Store the pod name associated with the received broker
					podName, found := brokerPodMap[receivedBroker]
					if !found {
						fmt.Printf("Pod name not found for broker '%s'. Aborting.\n", receivedBroker)
						ginkgo.Fail("Pod name not found for received broker")
					}

					// Print where the message was found
					fmt.Printf("Message found in broker '%s' (Pod: %s).\n", receivedBroker, podName)
		...
		// Step 4: Delete the broker pod that received the message
		deletePodName := brokerPodMap[receivedBroker]

...
var _ = Describe("Kafka Broker Pods", func() {
	It("should have the correct 'activemq-artemis-controller-manager-' prefixed pods running", func() {
		...
		expectedPodPrefix0 := "activemq-artemis-controller-manager-"
		...
		expectedPodCount := 1 
		Expect(actualPodCount).To(Equal(expectedPodCount), "Expected %d 'activemq-artemis-controller-manager-' pods, but found %d", expectedPodCount, actualPodCount)
	})
})

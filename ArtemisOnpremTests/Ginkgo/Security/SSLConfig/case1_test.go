...
var _ = Describe("Artemis SSL and AMQP Test", func() {
    ...
    BeforeEach(func() {
        ...
        caCert, err := ioutil.ReadFile("/etc/secret/pem/kafka-bundle.pem") // Replace with actual path to your CA cert
    ...
    It("should successfully connect", func() {
        // AMQP communication
        client, err := amqp.Dial("amqps://artemis-hdls-svc.artemistest.svc:61617", amqp.ConnSASLPlain("artemis", "artemis"), amqp.ConnTLSConfig(config))
        Expect(err).NotTo(HaveOccurred())
        defer client.Close()

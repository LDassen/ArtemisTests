...
var _ = Describe("Artemis SSL and AMQP Test", func() {
    ...
    BeforeEach(func() {
        ...
        caCert, err := ioutil.ReadFile("/etc/secret/kafka-bundle.pem") // Replace with actual path to your CA cert
    ...
    It("should successfully connect", func() {
        // AMQP communication
        client, err := amqp.Dial("amqps://artemis-statefulset-0.artemistest:61617", amqp.ConnSASLPlain("artemis", "artemis"), amqp.ConnTLSConfig(config))
        Expect(err).NotTo(HaveOccurred())
        defer client.Close()
